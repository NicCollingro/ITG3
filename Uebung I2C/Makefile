...
cleaner:	clean
	rm -rf *.qpf *.qsf sim.vvp simulation/
...
#	/bin/echo -e "$(vhdlcfg)" >> $(PROJECT).qsf
	/bin/echo -e "set_global_assignment -name EDA_SIMULATION_TOOL \"ModelSim-Altera (VHDL)\"\nset_global_assignment -name EDA_OUTPUT_DATA_FORMAT VHDL -section_id eda_simulation \nset_global_assignment -name EDA_TIME_SCALE \"1 ps\" -section_id eda_simulation \nset_global_assignment -name EDA_NETLIST_WRITER_OUTPUT_DIR simulation/modelsim -section_id eda_simulation \nset_global_assignment -name NUM_PARALLEL_PROCESSORS 4\n" >> $(PROJECT).qsf
